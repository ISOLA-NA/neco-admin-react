...
const CalendarTabs = forwardRef<CalendarHandle, CalendarProps>(({ selectedRow }, ref) => {
  const api = useApi();
  ...
  const [exceptionData, setExceptionData] = useState<{ [key: string]: number }>({});
  ...
  // Initialize data when selectedRow changes
  useEffect(() => {
    if (selectedRow) {
      setCalendarData({
        ...selectedRow,
      ...
      try {
        setRoutineData(JSON.parse(selectedRow.dateTimeRoutine || "{}"));
        setExceptionData(JSON.parse(selectedRow.SpecialDay || "{}"));
      } catch (error) {
        console.error("Error parsing stored data:", error);
        showAlert('error', null, 'Error', 'Error parsing calendar data');
      }
    } else {
      setCalendarData({
        Name: "",
  ...
  const handleRoutineChange = (day: string, e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const handleSaveModal = () => {
    ...
    if (isNaN(numValue) || numValue < 0 || numValue > 24) {
      showAlert('error', null, 'Error', 'Please enter a valid number between 0 and 24');
      return;
    }
  ...
  return (
    <div className="p-4 bg-white rounded-lg shadow-sm max-w-7xl mx-auto">
      {/* Name Input - Centered and Half Width */}
              ...
              {weekDays.map((day) => (
                <div key={day} className="flex items-center gap-4 bg-gray-50 p-4 rounded-lg">
                  <label className="flex items-center gap-2 min-w-[120px]">
                    ...
                    />
                    <span className="text-gray-700 whitespace-nowrap">{day}</span>
                  </label>
          ...
          <div className="exception-tab p-4 bg-white rounded-lg shadow-sm">
            ...
            <div className="grid grid-cols-7 gap-2">
              ...
              {[...Array(daysInMonth(month, year))].map((_, day) => {
                const dateStr = `${year}-${(month + 1).toString().padStart(2, '0')}-${(day + 1).toString().padStart(2, '0')}`;
                ...
                return (
                  <div
                    ...
                    className={`flex flex-col items-center p-2 rounded-lg cursor-pointer transition-all duration-200 ${
                      hasValue ? 'bg-gradient-to-r from-purple-600 to-indigo-500 text-white' : 'bg-white hover:bg-gray-100 border border-purple-600'
                    }`}
                    onClick={() => handleOpenModal(day + 1)}
                  >
                    <span className={hasValue ? 'text-white' : 'text-gray-700'}>
                      {day + 1}
                    </span>
                    {hasValue && (
