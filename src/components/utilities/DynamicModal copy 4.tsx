...
interface DynamicModalProps {
  ...
  selectedRowData: any; // افزودن این پراپ
  setSelectedRowData: (data: any) => void; // افزودن این پراپ
}
...
const DynamicModal: React.FC<DynamicModalProps> = ({
  ...
  const handleRowDoubleClick = (data: any) => {
    setSelectedRow(data); // ذخیره داده انتخاب‌شده
    onSelect(data); // ارسال داده به `onSelect`
    handleClose(); // بستن مدال
  };
  ...
  const handleSelectClick = () => {
    if (selectedRow) {
      onSelect(selectedRow); // ارسال داده به `onSelect`
      handleClose(); // بستن مدال
    }
  };
