...
import AppServices, { AlertingWfTemplateItem, Role } from '../../../services/api.services'
...
export default function AlertForm() {
  ...
  const [boxTemplates, setBoxTemplates] = useState<{ ID: number; Name: string }[]>([])
  ...
  useEffect(() => {
    ...
    AppServices.getAllBoxTemplatesByWfTemplateId(16).then(setBoxTemplates).catch(console.error)
  }, [])
  ...
  const handleAdd = async () => {
    const payload: AlertingWfTemplateItem = {
      ...
      SensitiveItemWFTemp: oldVersion ? parseInt(alertObj.SensitiveItemWFTemp, 10) : 0,
      SensitivityWFTemp: newVersion ? parseInt(alertObj.SensitivityWFTemp, 10) : 0,
      WFState: newVersion ? parseInt(alertObj.WFState, 10) : 0,
      SendType: parseInt(alertObj.sendType, 10),
      nPostTypeID: null,
      nPostID: alertObj.nPostID,
      nWFBoxTemplateId: parseInt(alertObj.nWFBoxTemplateId, 10),
  ...
  const columnDefs = [
    ...
    { headerName: 'Send Type', field: 'sendType' },
    {
      headerName: 'Receiver',
      field: 'nPostID',
      valueGetter: (p: any) => roles.find(r => r.ID === p.data.nPostID)?.Name || '',
    },
    {
      headerName: 'Box Template',
      field: 'nWFBoxTemplateId',
      valueGetter: (p: any) => boxTemplates.find(bt => bt.ID === +p.data.nWFBoxTemplateId)?.Name || '',
    },
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <select value={alertObj.sendType} onChange={handleChange('sendType')} className="w-full border rounded px-2 py-1">
          <option value="" hidden>
            Send Type
          </option>
        ...
        <select value={alertObj.nPostID} onChange={handleChange('nPostID')} className="w-full border rounded px-2 py-1">
          <option value="" hidden>
            Receiver
          </option>
          {roles.map(r => (
        ...
        <select
          value={alertObj.nWFBoxTemplateId}
