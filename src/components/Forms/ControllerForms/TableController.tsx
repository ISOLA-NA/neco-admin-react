...
const TableController: React.FC<TableControllerProps> = ({ onMetaChange, data = {} }) => {
  ...
  const computedHeader = useMemo(() => headerInput.replace(/\n/g, "|"), [headerInput]);
  ...
  /* ---------- sync props→state ---------- */
  useEffect(() => {
    setHeaderInput((p) => (p === (data.metaType1 ?? "a\nb\nc") ? p : data.metaType1 ?? "a\nb\nc"));
    setIsRowFixed((prev) => (prev === !!data.metaType2 ? prev : !!data.metaType2));
    setFixRowValue((p) => (p === (data.metaType2 ?? "") ? p : data.metaType2 ?? ""));
  ...
  useEffect(() => {
    const meta = {
      ...
      metaType3: JSON.stringify(
        tableData
          .filter((r) => ["a1", "a2", "a3"].some((k) => r[k]?.toString().trim()))
  ...
  /* ---------- handlers ---------- */
  const handleAddRow = () =>
    setTableData((p) => [...p, { id: nextRowId.current++, a1: "", a2: "", a3: "" }]);
